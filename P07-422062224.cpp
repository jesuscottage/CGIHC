...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// CARROZA
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f, 0.6f, 14.0f));
		...
		// FARO AZUL (SPOTLIGHT)
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, -0.2f, -4.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
        glm::vec3 faroPosition = glm::vec3(model[3]);
		...
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glm::vec3 farohelPosition = glm::vec3(model[3]);
		spotLights[2].SetPos(farohelPosition); // actualizar posición de la luz
